...
namespace LittlePhysics
{
    ...
    public partial struct FrictionSystem : ISystem
    {
        ...
        private struct ApplyFrictionJob : IJobParallelFor
        {
            ...
            {
                if (surfaceBody.RigidbodyData.Friction <= 0)
                {
                ...
                float frictionFactor = math.clamp(surfaceBody.RigidbodyData.Friction * DeltaTime, 0f, 1f);

                frictionLinear = -tangentialVelocity * frictionFactor;
                frictionAngular = velocity.IsRotationBlocked ? -velocity.Angular * frictionFactor : float3.zero;
            }
