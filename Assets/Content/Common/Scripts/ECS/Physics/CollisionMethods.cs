...
namespace LittlePhysics
{
    ...
    public static partial class CollisionMethods
    {
        ...
        /// <summary>
        /// Returns true when a capsule overlaps a horizontal infinite plane at plane.Y, tested from above only.
        /// Uses the lower of the two axis endpoints so the full capsule height is accounted for.
        /// Contact point is on the plane surface directly below the lowest axis endpoint.
        /// </summary>
        public static bool IsCapsuleCollidingSimplePlane(Capsule capsule, SimplePlane plane, out float3 contactPoint)
        {
            contactPoint = float3.zero;
            ...
            float3 capA = capsule.Position - capsule.Up * 0.5f;
            float3 capB = capsule.Position + capsule.Up * 0.5f;
            float3 lowestCap = capA.y <= capB.y ? capA : capB;

            if (lowestCap.y - radius > plane.Y + CollisionEpsilon)
                return false;

            contactPoint = new float3(lowestCap.x, plane.Y, lowestCap.z);
            return true;
        }
        ...
        public static bool IsLineCollidingBody(Line line, PhysicsBodyData body, out float3 contactPoint)
        {
            ...
            {
                return IsLineCollidingCapsule(line, new Capsule { Position = body.PositionData.Position, Up = body.PositionData.Up, Scale = body.PositionData.Scale }, out contactPoint);
            }

            return IsLineCollidingSphere(line, new Sphere { Position = body.PositionData.Position, Scale = body.PositionData.Scale }, out contactPoint);
        }
        ...
        public static bool AreBodiesColliding(PhysicsBodyData body1, PhysicsBodyData body2, out float3 contactPoint)
        {
            switch (body1.ShapeType, body2.ShapeType)
            {
                ...
                case (ShapeType.Capsule, ShapeType.SimplePlane):
                    return IsCapsuleCollidingSimplePlane(body1.GetCapsule(), body2.GetSimplePlane(), out contactPoint);
                ...
                case (ShapeType.SimplePlane, ShapeType.Capsule):
                    return IsCapsuleCollidingSimplePlane(body2.GetCapsule(), body1.GetSimplePlane(), out contactPoint);
